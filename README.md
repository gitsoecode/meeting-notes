...
| Audio | Pick the microphone input device. Toggle Apple voice processing — Apple's built-in echo cancellation + AGC + noise suppression on the mic. Off by default; the engine's offline AEC handles most setups. Turn on if your built-in speakers bleed heavily into the mic and the offline pass isn't enough. |
