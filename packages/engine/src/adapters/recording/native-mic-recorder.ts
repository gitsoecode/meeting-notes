...
export function startNativeMic(options: NativeMicSessionOptions): NativeMicSession {
  ...
  const child: ChildProcess = spawn(options.binaryPath, [options.outputPath], {
    ...
    // VPIO is on by default; the Settings → Audio toggle drives this
    // flag. Always set the env var explicitly: "1" when disabled,
    // undefined when enabled. Setting an env value to undefined causes
    // Node's spawn to drop the key from the child's environment, so a
    // stray GISTLIST_DISABLE_VOICE_PROCESSING=1 inherited from the
    // parent shell can't override the UI when the user has the toggle
    // on.
    env: {
      ...process.env,
